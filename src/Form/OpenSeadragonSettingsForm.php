...
class OpenSeadragonSettingsForm extends ConfigFormBase {
   ...
   *   The HTTP client.
   */
  public function __construct(ConfigFactoryInterface $config_factory, ConfigInterface $seadragon_config, ClientInterface $http_client) {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['image_server_settings'] = [
      '#type' => 'details',
      '#title' => $this->t('IIIF Server Settings'),
      '#open' => TRUE,
      'iiif_server' => [
        '#type' => 'textfield',
        ...
        '#required' => TRUE,
        '#description' => $this->t('Please enter the image server location without trailing slash. eg:  http://www.example.org/iiif/2.'),
        '#config' => [
      ...
      'manifest_view' => [
        '#type' => 'select',
        '#title' => $this->t('IIIF Manifest View'),
        '#empty_value' => TRUE,
        '#default_value' => $this->seadragonConfig->getManifestView(),
        '#description' => $this->t('If using a view to generate IIIF manifests, please select it here.'),
        '#options' => Views::getViewsAsOptions(TRUE),
      ],
    ];
    $form['openseadragon_settings'] = [
      '#type' => 'details',
      '#title' => $this->t('OpenSeadragon Settings'),
      '#open' => TRUE,
      '#tree' => TRUE,
      'fit_to_aspect_ratio' => [
        '#type' => 'checkbox',
      ...
      'compositeOperation' => [
        ...
        '#description' => $this->t('How the image is composited onto other images.'),
      ...
      'maxImageCacheCount' => [
        '#type' => 'textfield',
        '#title' => $this->t('Max Image Cache Count'),
        '#size' => 10,
        '#element_validate' => [[$this, 'elementValidateNumber']],
      ...
      'sequenceOptions' => [
        ...
        'sequenceMode' => [
          '#type' => 'item',
          '#description' => $this->t('Default mode if multiple images are detected.  Images are viewed one at a time with arrow buttons for navigation. Enabling Collection Mode will disable Sequence Mode.'),
        ],
        'sequenceContainer' => [
          '#type' => 'container',
          '#description' => $this->t('Default mode if multiple tile sources are to be displayed.  Images will be viewed one at a time with arrow buttons for navigation.  Enabling Collection Mode will override Sequence Mode.'),
          '#states' => [
            'enabled' => [
      ...
      'collectionModeFields' => [
        ...
        'collectionMode' => [
          ...
          '#description' => $this->t('Arranges multiple images in a grid or line. Enabling Collection Mode will disable Sequence Mode.'),
        ],
